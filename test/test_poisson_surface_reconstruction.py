from .context import gpytoolbox
from .context import numpy as np
from .context import unittest
import matplotlib.pyplot as plt


class TestPoissonSurfaceReconstruction(unittest.TestCase):
    def test_indicator(self):
        np.random.seed(0)
        # First test: "uniform" sampling density
        # Sample points on a circle
        th = 2*np.pi*np.random.rand(10,1)
        P = np.concatenate((np.cos(th) + 0.1,np.sin(th) + 0.2),axis=1)
        # Normals are the same as positions on a circle
        N = np.concatenate((np.cos(th),np.sin(th)),axis=1)

        corner = np.array([-1.5,-1.5])
        gs = np.array([30, 30])
        h = np.array([0.1,0.1])

        scalar_mean, scalar_var = gpytoolbox.poisson_surface_reconstruction(P,N,gs=gs,h=h,corner=corner)
        # plt.imshow(scalar_var.reshape(gs,order='F'))
        # plt.colorbar()
        # plt.imshow(scalar_mean.reshape(gs,order='F'))
        # plt.colorbar()
        # plt.show()
        

if __name__ == '__main__':
    unittest.main()