...
def signed_distance(Q,V,F=None,use_cpp=True,aabb=None,fwn_bvh=None):
    ...
    `aabb=` and `fwn_bvh=` to avoid the O(n) construction cost on every
    call:
    ```python
