...
def unsigned_distance(Q,V,F=None,use_cpp=True,cpp_aabb=None):
    ...
    cpp_aabb : gpytoolbox.squared_distance_precompute, optional (default None)
        Precomputed AABB tree built via `gpytoolbox.squared_distance_precompute(V, F)`.
        Only used when `use_cpp=True`. Reuses the tree across calls instead of
        rebuilding it, avoiding the O(n) construction cost on every query.
    ...
    See Also
    --------
    squared_distance, squared_distance_precompute
    """
